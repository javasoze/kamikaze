...
public class PForDeltaAndDocIdSet extends ImmutableDocSet implements Serializable {
  ...
  class PForDeltaAndDocIdSetIterator extends DocIdSetIterator {
    ...
    public final int nextDoc() throws IOException {
      ...
      // hy: if it reaches the end of the shortest list, do not scan other lists
      ...
      while (i < size) {
        ...
        if (i != skip) {
          ...
          if (docId > target) { // hy: cannot find the target in the next list
            ...
            else // hy: for the first list, it must succeed as long as the docId is not NO_MORE_DOCS
              skip = 0;
          }
        }
        i++;
